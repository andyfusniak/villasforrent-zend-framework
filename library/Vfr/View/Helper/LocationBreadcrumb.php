...
class Vfr_View_Helper_LocationBreadcrumb extends Zend_View_Helper_Abstract
{
    ...
    const DEFAULT_SHOW_ROOT_NODE = true;
    ...
    public function locationBreadcrumb($idLocation, $options=null)
    {
        ...
        if (($options) && (is_array($options))) {
            ...
            // root node is World with link to home page
            if (isset($options['showRootNode']))
                $showRootNode = $options['showRootNode'];
            else
                $showRootNode = self::DEFAULT_SHOW_ROOT_NODE;
        } else {
            $whitespace = self::DEFAULT_WHITESPACE;
            $delimiter  = self::DEFAULT_DELIMITER;
            $totalsMode = self::DEFAULT_TOTAL_MODE;
        ...
        if ($showRootNode) {
            if ($makeLinks)
                $xhtml .= '<a class="breadcrumb" href="/">World</a>';
            else
                $xhtml .= $name;
