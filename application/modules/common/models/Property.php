...
class Common_Model_Property extends Vfr_Model_Abstract
{
    ...
    public function getAllFeaturedPropertiesPaginator($page = 1, $interval = 30, $order = 'F.idProperty, L.name', $direction = 'ASC')
    {
        $session = new Zend_Session_Namespace(
            Common_Model_Property::SESSION_NS_ADMIN_PROPERTY_LIST_BY_PROPERTY
        );
        ...
        $lang='EN', $idPropertyContentFieldList=null)
    {
        ...
        $propertyContentResource = $this->getResource('PropertyContent');
        return $propertyContentResource->getPropertyContentByPropertyId(
            $idProperty, $version, $lang, $idPropertyContentFieldList
        );
    }
        ...
        $lang='EN', $idPropertyContentFieldList=null)
    {
        if ((!$properties instanceof Common_Resource_Property_Rowset)
            ...
            && (!$properties instanceof Zend_Paginator)
            && (! is_array($properties))) {
    ...
    public function getAvailabilityRangeByCalendarId($idCalendar, $startDate = null, $endDate = null)
    {
        ...
        return $availabilityResource->getAvailabilityRangeByCalendarId(
            $idCalendar,
            $startDate,
            $endDate
        );
    ...
    public function copyUpdateToMaster($idProperty)
    {
        ...
        // notify the advertser that their property updates have been approved
        $vfrMail = new Vfr_Mail(
            '/modules/controlpanel/views/emails',
            'advertiser-property-update-approved' // no extensions required
        );
        ...
        $vfrMail->send(
            "andy@mars.local",
            "HolidayPropertyWorldwide.com Property Update Approved",
            array(
    ...
    public function setAwaitingApproval($idProperty)
    {
        ...
        //
        // notify the administrator that this property needs initial approval
        ...
        if ($notifyAdmin) {
            $vfrMail = new Vfr_Mail(
                '/modules/controlpanel/views/emails',
                'initial-approval-notify-admin' // no extensions required
            );
            ...
            $vfrMail->send(
                $adminEmail,
                "HolidayPropertyWorldwide.com Property Requiring Initial Approval",
                array (
                    'idProperty' => $idProperty
                ),
                Vfr_Mail::MODE_ONLY_TXT
    ...
    public function initialApproveProperty($propertyRow)
    {
        ...
        $idAdvertiser = (int) $propertyRow->idAdvertiser;

        $propertyResource = $this->getResource('Property');
        $propertyResource->initialApproveProperty($idProperty);

        $advertiserResource = $this->getResource('Advertiser');
        $advertiserRow = $advertiserResource->getAdvertiserById($idAdvertiser);

        //
        // notify the advertiser that their property has been approved
        $vfrMail = new Vfr_Mail(
            '/modules/controlpanel/views/emails',
            'advertiser-property-approved' // no extensions required
        );

        $vfrMail->send(
            $advertiserRow->emailAddress,
            //'andy@mars.local',
            "HolidayPropertyWorldwide.com Your property has been approved",
            array (
                'idProperty'  => $idProperty,
                'firstname'   => $advertiserRow->firstname,
    ...
    public function advertiserSendForUpdateApproval($idProperty)
    {
        ...
        //
        // notify the administrator that this property needs update approval
        // only if the application.ini says
        // vfr.mail.sendAdminTransactionalEmails = Yes
        //
        ...
        if ($notifyAdmin) {
            $vfrMail = new Vfr_Mail(
                '/modules/controlpanel/views/emails',
                'update-approval-notify-admin' // no extensions required
            );
            ...
            $vfrMail->send(
                $adminEmail,
                "HolidayPropertyWorldwide.com Property Requiring Update Approval",
                array (
                    'idProperty' => $idProperty
                ),
                Vfr_Mail::MODE_ONLY_TXT
