	...
	public function copyUpdateToMaster($idProperty)
	{
		...
		$query = $adapter->quoteInto("
			UPDATE PropertiesContent AS c
                ...
                (
                SELECT DISTINCT
                    t1.idPropertyContent, t1.version, t1.idPropertyContentField, t1.idProperty, t2.content, t2.cs
                FROM
                ...
                WHERE
                    t1.version=1 AND t2.version=2 AND t1.iso2char='EN' AND t1.idProperty=?
                )
                AS tm
                ON
                    tm.idPropertyContent=c.idPropertyContent
                SET
                    c.content=tm.content, c.updated=now()
		", $idProperty);
